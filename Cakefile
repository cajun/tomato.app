...
task 'assets:watch', 'Watch source files and build JS', (options) ->
  ...
  runCommand './node_modules/.bin/coffee', '-wc', '-o', 'source', 'coffee'
...
task 'build', 'Build Coffee and Less files into js and css files', ->
  ...
  lessc = spawn( './node_modules/.bin/lessc', [ 'less/tomato.less', 'css/tomato.css' ] )
  out_process lessc

task 'docs', 'Build the docco docs', ->
  exec './node_modules/.bin/docco coffee/*.coffee', (error,stdout,stderr) ->
    util.print error
    util.print stdout
    util.print stderr
